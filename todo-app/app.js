(() => {
  ...
  const Store = {
    async load() {
      try {
        ...
        console.warn('读取 localStorage 失败，将加载默认数据。', err);
      }
      try {
        const response = await fetch('data/todos.json', { cache: 'no-store' });
        if (response.ok) {
          const data = await response.json();
          return this.migrate(data);
        }
      } catch (err) {
        console.warn('读取默认数据文件失败，使用初始状态。', err);
      }
  ...
  function updateStorageStatus() {
    ...
    dom.storageStatus.textContent = hasHydrated
      ? '数据会自动保存，可导出为 JSON 备份到 data/todos.json。'
      : '正在加载数据…';
  }
  ...
  async function initialize() {
    ...
    updateStorageStatus();
    if (dom.newTodoInput) {
      dom.newTodoInput.disabled = true;
    }
    const submitBtn = dom.newTodoForm ? dom.newTodoForm.querySelector('button[type="submit"]') : null;
    if (submitBtn) {
    ...
    hasHydrated = true;
    if (dom.newTodoInput) {
      dom.newTodoInput.disabled = false;
    }
    if (submitBtn) {
